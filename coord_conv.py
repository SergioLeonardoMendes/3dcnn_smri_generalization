...
class _CoordinateChannel(Layer):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        if self.rank == 3:
            ...
            xx_ones = tf.ones(K.stack([batch_shape, dim3]), dtype='int32') #LEOTMP
            xx_ones = K.expand_dims(xx_ones, axis=-1)
            ...
            yy_ones = tf.ones(K.stack([batch_shape, dim2]), dtype='int32') #LEOTMP
            yy_ones = K.expand_dims(yy_ones, axis=1)
            ...
            zz_channels = tf.dtypes.cast(zz_channels, tf.float16)  #LEOTMP
            xx_channels = tf.dtypes.cast(xx_channels, tf.float16)  #LEOTMP
            yy_channels = tf.dtypes.cast(yy_channels, tf.float16)  #LEOTMP
            inputs = tf.dtypes.cast(inputs, tf.float16)  #LEOTMP
